...
fun queenThreatens(x1: Int, y1: Int, x2: Int, y2: Int): Boolean {
    return when {
        x1 == x2 || y1 == y2 || kotlin.math.abs(x1 - x2) == kotlin.math.abs(y1 - y2) -> return true
        else -> false
    }
}
...
fun circleInside(x1: Double, y1: Double, r1: Double,
    ...
    return when {
        (kotlin.math.sqrt(sqr(x2 - x1) + sqr(y2 - y1)) + r1) <= r2 -> true
        else -> false
    }
}
...
fun brickPasses(a: Int, b: Int, c: Int, r: Int, s: Int): Boolean = when {
    (a <= r && b <= s) || (a <= s && b <= r) || (a <= r && c <= s) || (a <= s && c <= r) || (b <= r && c <= s) || (b <= s && c <= r) -> true
    else -> false
}
